...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):     
    ...
    def get_token(cls, user):
        token = super().get_token(user)                 # here the username is derived from the token that is generated
...
class MyTokenObtainPairView(TokenObtainPairView):            # created new view by inheriting from the original view
...
def getRoutes(request):
    routes = [
        '/api/token',                      #for username and password
        '/api/token/refresh',              # gives you a new token if you refresh
    ]
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])         # it is from the rest frame work
def getNotes(request):
    user = request.user
    notes = user.note_set.all()                # we will get all the notes of that perticular user
